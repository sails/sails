...
static inline bool IsPathValid(char* path)
{
  ...
  if (stat(path, &statbuf))
  {
    log::LoggerFactory::getLog("server")->error(
        "stat error, permission problem, path=%s, error=%s",path, strerror(errno));
    return false;
  }
...
static bool OpenAndReadFile(char* filename, char* buffer, int buff_len, int* bytes)
{
  ...
  if (fd == -1)
  {
    log::LoggerFactory::getLog("server")->error("open file error, filename=%s", filename);
    return false;
  }
  ...
  *bytes = read(fd, buffer, buff_len - 1);
  ...
  if (*bytes <= 0)
  {
    log::LoggerFactory::getLog("server")->error("read file error, bytes:%d", *bytes);
    return false;
  }
...
static bool StringToUINT64(const std::string& str, uint64_t* value ) {
  long int temp = strtol(str.c_str(), (char**)NULL, 10);
  if (temp >= 0) {
    *value = temp;
  }else {
    log::LoggerFactory::getLog("server")->error(
...
bool GetThreadCpuTime(int32_t pid, int tid, uint64_t* thread_cpu_time)
{
  ...
  if (!ret)
  {
    log::LoggerFactory::getLog("server")->error("get param error, fields size=%u", fields.size());
    return false;
  }
