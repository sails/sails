...
#include <common/crypto/MD5Checksum.h>
#include <common/crypto/MD5ChecksumDefines.h>
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
...
*****************************************************************************************/
std::string CMD5Checksum::GetMD5(const BYTE* pBuf, UINT nLength)
{
...
bool CMD5Checksum::GetTruncatedMD5(BYTE* pBuf, 
                                   UINT nLength, 
                                   ...
                                   UINT iSize /* = 16 */)
{
    ...
    CMD5Checksum MD5Checksum;
    MD5Checksum.Update( pBuf, nLength );
    return MD5Checksum.Final(vecOut, iSize);
}
...
std::string CMD5Checksum::Final()
{
    ...
    for ( int i=0; i < nMD5Size; i++) 
    {
        ...
        else if (lpszMD5[i] <= 9 && lpszMD5[i] > 0)
        {
            szBuffer[0] = '0';
            szBuffer[1] = '0' + lpszMD5[i];
            szBuffer[2] = '\0';
            Str = Str + szBuffer;
            //Str.Format("0%x",lpszMD5[i]);
        }
        else if (lpszMD5[i] <= 15 && lpszMD5[i] > 9)
        {
            szBuffer[0] = '0';
            szBuffer[1] = 'a' + lpszMD5[i]-10;
            szBuffer[2] = '\0';
            Str = Str + szBuffer;
        }
        else 
        {
            //Str = Str + itoa(lpszMD5[i], szBuffer, 16);
	    sprintf(szBuffer, "%x", lpszMD5[i]);
	    Str = Str+szBuffer;
...
bool CMD5Checksum::Final(std::vector<BYTE>& vecOut_, UINT iSize_)
{
    vecOut_.clear();
    if( iSize_ > 16 )
        return false;
    ...
    vecOut_.resize(iSize_);
    memcpy(&vecOut_[0], lpszMD5, iSize_);
